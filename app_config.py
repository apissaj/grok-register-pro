...
DEFAULT_CONFIG = {
    ...
    "close_browser_between_accounts": False,
...
def validate_config_structure(raw):
    ...
    path_keys = {"grok2api_local_token_file", "api_reverse_tools", "cpa_auth_dir", "cpa_hotload_dir", "grok2api_auth_dir"}
    for key in string_keys:
        cfg[key] = _require_string(cfg, key, path=key in path_keys)
    enums = {
        "email_provider": {"duckmail", "yyds", "cloudflare", "cloudmail", "tempik", "mailnest", "moemail"},
        "cloudflare_auth_mode": {"query-key", "bearer", "x-api-key", "x-admin-auth", "none"},
        "grok2api_pool_name": {"ssoBasic", "ssoSuper"},
        "cpa_token_mode": {"device_protocol", "device_browser", "auth_code"},
    }
    ...
    url_keys = {
        ...
        "grok2api_remote_base", "cpa_base_url", "moemail_api_base",
        "cloudmail_url", "cpa_remote_url", "duckmail_api_base",
    }
    for key in url_keys:
        value = cfg[key]
        if not value:
...
def validate_run_requirements(cfg):
    ...
    if provider == "mailnest" and not cfg["mailnest_api_key"]:
        raise ConfigError("MailNest 模式需要配置 mailnest_api_key")
    if provider == "moemail" and not cfg["moemail_api_base"]:
        raise ConfigError("MoeMail 模式需要配置 moemail_api_base")
