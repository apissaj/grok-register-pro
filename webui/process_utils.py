...
def _windows_long_path(path: str | os.PathLike[str]) -> str:
    """Expand Windows 8.3 short paths (e.g. TUFGAM~1) to long form.

    psutil returns process cwd in 8.3 short form on Windows, while Python's
    Path.resolve() does NOT expand 8.3 components — so the two never compare
    equal. GetLongPathNameW normalizes both sides to the same long form.
    """
    if os.name != "nt":
...
def _process_snapshot(process) -> dict | None:
    try:
        ...
        if not cwd_value or not isinstance(cmdline_value, (list, tuple)):
            return None
        cwd = Path(_windows_long_path(str(cwd_value))).resolve()
...
def _snapshot_matches(
    ...
    project_root = Path(_windows_long_path(os.fspath(root))).resolve()
