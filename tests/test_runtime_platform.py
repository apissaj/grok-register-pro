...
def test_linux_display_and_disabled_mode_launch_directly():
    with tempfile.TemporaryDirectory() as temp:
        ...
        assert with_display[0] == str(python.resolve())
        assert explicitly_disabled[0] == str(python.resolve())
...
def test_macos_and_windows_launch_without_xvfb():
    with tempfile.TemporaryDirectory() as temp:
        ...
        assert mac_command[0] == str(mac_python.resolve())
        assert windows_command[0] == str(windows_python.resolve())
