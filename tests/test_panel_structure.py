...
def test_help_and_faq_module():
    ...
    assert 'id="help-view-label" aria-hidden="true">Bantuan</span>' in html
    assert 'aria-label="Buka Bantuan"' in html
    assert 'label.textContent = isHelp ? "Kembali" : "Bantuan";' in mon
    ...
    assert 'Recovery Akun' in html
    assert 'akun yang sudah punya CPA dilewati' in html
    ...
    assert 'showHelpFor("token")' in mon
    assert 'Token akses tidak cocok, silakan masukkan ulang token panel' in mon
    assert '—' not in html
    assert '–' not in html
...
def test_email_service_and_domain_rotation_panel_structure():
    ...
    assert 'id="domain-view-label" aria-hidden="true">Email</span>' in html
    ...
    assert 'Rotasi Domain <span class="domain-advanced-meta">Pengaturan Lanjutan</span>' in html
...
def test_panel_security_and_recovery_structure():
    ...
    assert 'id="run-status" aria-label="Status tugas: memuat"' in html
    assert 'setAttribute("aria-label", "Status tugas:" + runLabel)' in mon
    assert 'id="kpis" aria-label="Metrik Utama" aria-live=' not in html
