...
from camoufox_adapter import CamoufoxBrowser, CamoufoxPage, set_win_driver_pid
from camoufox_adapter import _force_foreground_windows as _adapter_force_focus
...
def _force_foreground_camoufox(driver_pid: int) -> None:
    """Windows-only: force-show and focus the Camoufox (Firefox) browser windows.

    When Camoufox launches from a background process, its windows are created
    WITHOUT the WS_VISIBLE style and never get DWM focus (Firefox defers
    layout/render and even DOM construction for invisible windows). Result:
    blank/white page until the user clicks the window, and automation sees
    `inputs=none; buttons=none` because the SPA never builds its DOM.

    This enumerates every top-level window owned by the driver process tree,
    force-shows it (WS_VISIBLE), restores it, brings it to front, and sets
    foreground focus — equivalent to a user click, but scripted.
    """
    if os.name != "nt":
        return
    try:
        ...
        pids = {driver_pid} | {ch.pid for ch in children}
        ...
        def _enum_cb(hwnd, _lparam):
            ...
            if pid.value in pids:
                ...
                if not (style & 0x40000000):  # skip WS_CHILD
                    found.append(hwnd)
            return 1
        ...
        for hwnd in found:
            try:
                # Show window (SW_SHOWNORMAL=1), update, restore, focus
        ...
        # Non-fatal: if we can't force foreground, the flow still works
        # (just slower / requires manual click).
        pass
...
def create_browser_options(unique_profile=True) -> dict:
    ...
    反检测策略：
    - headless: 从 config.json browser_headless 读取（默认 False 有头模式）
      有头模式（headless 更易被检测）但 Windows 后台进程需要点击才渲染；
      设 browser_headless=true 则完全无窗口、无需焦点、可多 worker 并行。
    ...
    _headless = False
    try:
        _cfg_path = _Path(__file__).resolve().parent / "config.json"
        if _cfg_path.exists():
            _headless = bool(_json.loads(_cfg_path.read_text(encoding="utf-8")).get("browser_headless", False))
    except Exception:
        pass

    opts: dict = {
        "headless": _headless,  # 反检测：有头模式（headless 更易被检测）
...
def start_browser(log_callback=None) -> Tuple[object, object]:
    ...
    for attempt in range(1, 5):
        ...
        try:
            ...
            # Tell adapter whether we're in headless mode (skip foreground focus)
            from camoufox_adapter import set_headless as _set_headless_adapter
            _set_headless_adapter(bool(opts.get("headless", False)))
            ...
            # Force Camoufox windows to foreground (Windows DWM fix)
            if os.name == "nt":
                try:
                    _driver_pid = getattr(
                        getattr(
                            getattr(camoufox, "_connection", None),
                            "_transport", None,
                        ),
                        "_proc",
                        None,
                    )
                    if _driver_pid is not None:
                        set_win_driver_pid(_driver_pid.pid)
                        _force_foreground_camoufox(_driver_pid.pid)
                        _adapter_force_focus()
                except Exception:
                    pass
