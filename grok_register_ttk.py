...
# ── 兼容旧版 grok2api 池入池适配器（可选） ──
def _log_exception_for_adapter(context, exc, log_callback=None):
...
def _grok2api_pools_adapter(raw_token, email="", log_callback=None):
    """把路由逻辑委托给旧版 account_outputs 时使用的薄适配层。
    仅在 import 成功并配置启用时生效；否则返回全部禁用的结果。"""
    _none = {
    ...
    _ao.configure_token_runtime(
        ...
        http_get,
        http_post,
        _log_exception_for_adapter,
        compatibility_error=RemoteTokenCompatibilityError,
        request_error=RemoteTokenRequestError,
    )
    return _ao.add_token_to_grok2api_pools(raw_token, email=email, log_callback=log_callback)
...
from email_providers import tempik as tempik_provider
...
DEFAULT_CONFIG = {
    ...
    # 兼容旧版 grok2api token 池（local token.json / remote app-key）
...
def _build_request_kwargs(**kwargs):
    ...
    # Browser UA: some Cloudflare-protected mail endpoints return 403/1010 without it
...
def get_tempik_api_base():
    return tempik_provider.normalize_base(
        str(os.environ.get("TEMPIK_API_BASE") or config.get("tempik_api_base", "") or "")
    )


def get_tempik_default_domain():
    return str(config.get("tempik_default_domain", "") or "").strip().lstrip("@")


def tempik_get_email_and_token():
    return tempik_provider.create_mailbox(
        http_get,
        http_post,
        get_tempik_api_base(),
        domain=get_tempik_default_domain(),
        username=generate_username(10),
    )
...
def tempik_get_oai_code(
    ...
    return tempik_provider.wait_for_code(
        http_get,
        get_tempik_api_base(),
        session_id,
        email,
...
class GrokRegisterGUI:
    ...
    def setup_ui(self):
        ...
        self.email_provider_combo = tk_option_menu(
            ...
            ["duckmail", "yyds", "cloudflare", "mailnest", "cloudmail", "moemail", "tempik"],
            width=12,
        )
    ...
    def start_registration(self):
        ...
        if config["email_provider"] == "cloudmail":
            ...
            if missing:
                ...
                return
        if config["email_provider"] == "tempik":
            if not get_tempik_api_base():
                self.log("[!] Tempik 模式需要先填写 Tempik API Base")
                return
            if not get_tempik_default_domain():
                self.log("[!] Tempik 模式需要先填写 Tempik 默认域名")
                return
        ...
        try:
            ...
            while i < count:
                ...
                try:
                    ...
                    # 兼容旧版 grok2api token 池（local token.json / remote app-key）
                    try:
                        pools = _grok2api_pools_adapter(sso, email=email, log_callback=wlog)
                        for _pname, _pstate in (pools or {}).items():
                            if isinstance(_pstate, dict) and _pstate.get("enabled") and not _pstate.get("ok"):
                                wlog(f"[!] grok2api {_pname} 入池失败: {_pstate.get('error')}")
                    except Exception as _pool_exc:
                        wlog(f"[!] grok2api 池入池后处理异常（账号已保留）: {_pool_exc}")
        ...
        while i < count:
            ...
            if wait_sec > 0:
                ...
                while time.time() < deadline:
                    ...
                    time.sleep(1)
            try:
                # Fast mode: keep browser alive between accounts to avoid
                # ~10s restart penalty per account. open_signup_page() will
                # navigate the existing browser to the signup page.
                if not config.get("close_browser_between_accounts", False):
                    # Fast mode: keep browser alive but clear session state so
                    # next account starts clean (cookies/localStorage cleared).
                    try:
                        _brows = active_browser()
                        if _brows is not None:
                            _ctx = getattr(_brows, "context", None) or getattr(_brows, "browser_context", None)
                            if _ctx is not None:
                                try:
                                    cdps = getattr(_ctx, "clear_cookies", None)
                                    if callable(cdps):
                                        cdps()
                                except Exception:
                                    pass
                                try:
                                    st = getattr(_ctx, "clear_storage_state", None)
                                    if callable(st):
                                        st()
                                except Exception:
                                    pass
                    ...
                    _next_proxy = None
                    try:
                        _next_proxy = pick_proxy_for_worker(0, single_rotate_idx)
                        set_thread_proxy(_next_proxy)
                    except Exception:
                        _next_proxy = None
                    if _next_proxy:
                        cli_log(f"[*] 下号代理: {redact_proxy(_next_proxy) or '直连'}")
                    continue
                stop_browser()
                time.sleep(0.5)
