...
# Windows-only: driver PID whose browser windows must be kept foreground.
# Thread-local because each worker runs in its own thread with its own
# Camoufox browser instance.
_win_tls = threading.local()
_is_headless = False

def set_win_driver_pid(pid: Optional[int]) -> None:
    _win_tls.driver_pid = pid

def set_headless(headless: bool) -> None:
    global _is_headless
    _is_headless = headless

def _force_foreground_windows() -> None:
    """Force-show + focus any hidden Camoufox window (Windows DWM fix).
    Skipped entirely in headless mode (no window exists)."""
    if _is_headless:
        return
    pid = getattr(_win_tls, "driver_pid", None)
    ...
    try:
        ...
        pids = {pid} | {ch.pid for ch in children}
...
class CamoufoxPage:
    ...
    def get(self, url: str, **kw):
        ...
        # Re-focus window after navigation (Firefox defers layout for
        # unfocused windows on Windows, causing blank page / empty DOM)
        try:
