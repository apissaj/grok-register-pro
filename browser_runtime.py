...
def create_browser_options(browser_proxy="", extension_path=None):
    ...
    # BLOCK ALL prompts (network access, notification, download, camera, mic, etc.)
    # Mirrors the stable QwenCloud browser args, plus explicit permission prefs.
    options.set_argument("--disable-blink-features", "AutomationControlled")
    options.set_argument("--disable-notifications", "true")
    options.set_argument("--disable-domain-reliability", "true")
    options.set_argument("--disable-web-security", "true")
    options.set_argument("--disable-site-isolation-trials", "true")
    options.set_argument("--allow-running-insecure-content", "true")
    options.set_argument("--unsafely-disable-site-isolation-trials", "true")
    options.set_argument("--no-sandbox")
    options.set_pref("privacy.local_network_sharing_prompt_disabled", True)
    options.set_pref("profile.default_content_setting_values.camera", 2)
    options.set_pref("profile.default_content_setting_values.microphone", 2)
